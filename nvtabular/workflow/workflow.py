...
import nvtabular
from nvtabular.columns import ColumnSchemaSet
...
class Workflow:
    ...
    def fit_schema(self, input_schema: ColumnSchemaSet):
        schemaless_nodes = {
        ...
        while stat_ops:
            ...
            for workflow_node in current_phase:
                ...
                if workflow_node.parents:
                    ...
                    if addl_input_cols:
                        # transformed_ddf = _concat_columns(
                        #   [transformed_ddf, ddf[list(addl_input_cols)]]
                        # )
                        transformed_ddf = dask.dataframe.multi.concat(
                            [transformed_ddf, ddf[list(addl_input_cols)]],
                            axis=1,
                        )
